...
export class AudiobooksService {
  ...
  private async resolveFilePath(relativePath: string): Promise<string> {
    ...
    return resolveContainedPath(audiobookLibraryPath, relativePath);
  }
  ...
  async findById(id: string): Promise<AudiobookDetailDto> {
    ...
    const [
      ...
      ab.generatedFromEbookId
        ? this.db
            ...
            .where(eq(ebookSchema.ebooks.id, ab.generatedFromEbookId))
            .limit(1)
        : Promise.resolve([]),
    ]);
    ...
    // External authors use canonical people UUIDs created during metadata import.
    const resolvedAuthors =
      resolvedAuthorNames === hardcoverAuthorNames && hc
        ? hardcoverAuthorNames.flatMap((name) => {
            const person = canonicalExternalPeople.get(
    ...
    // External series use canonical series UUIDs created during metadata import.
    const resolvedSeries =
      resolvedSeriesNames === hardcoverSeriesName && hc?.featuredSeriesName
        ? (() => {
            const series = canonicalExternalSeries.get(
    ...
    return {
      ...
      goodreads: gr
        ...
        : null,
      // Source ebook when this audiobook was AI-narrated from one
      generatedFromEbook: generatedFromEbookData[0] ?? null,
    };
  }
  ...
  private async updateSeries(
    ...
    for (const entry of seriesEntries) {
      ...
      let [seriesRecord] = await this.db
        .select()
        .from(schema.series)
        .where(sql`LOWER(${schema.series.name}) = LOWER(${name})`)
        .limit(1);
