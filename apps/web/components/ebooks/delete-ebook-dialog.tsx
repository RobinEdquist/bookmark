...
export function DeleteEbookDialog({
  ebookId,
  ebookTitle,
  open,
  onOpenChange,
}: DeleteEbookDialogProps) {
  const t = useTranslations("ebooks.deleteDialog");
  const [deleteOption, setDeleteOption] = useState<"keep" | "delete">("keep");
  const { mutateAsync: deleteEbook, isPending } = useDeleteEbook();
