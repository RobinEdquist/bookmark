...
import { useUpdateCover } from "../../lib/use-audiobooks";
...
export function ChangeCoverDialog({
  ...
  const { mutateAsync: updateCover, isPending } = useUpdateCover();
  ...
  const handleSubmit = async () => {
    try {
      if (activeTab === "upload" && selectedFile) {
        await updateCover({ audiobookId, file: selectedFile });
      } else if (activeTab === "url" && urlInput.trim()) {
        await updateCover({ audiobookId, url: urlInput.trim() });
      }
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <Tabs defaultValue="upload" value={activeTab} onValueChange={(v) => setActiveTab(v as "upload" | "url")}>
          ...
          <TabsContent value="url" className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="cover-url">{t("url.label")}</Label>
