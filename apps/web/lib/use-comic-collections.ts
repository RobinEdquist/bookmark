...
async function fetchCollections(
  f: ComicCollectionFilters = {},
): Promise<{ collections: ComicCollectionListItem[]; total: number }> {
  const r = await fetch(`/api/comics/collections?${buildParams(f)}`, {
    credentials: "include",
  });
...
export function useInfiniteComicCollections(
  f: Omit<ComicCollectionFilters, "limit" | "offset"> = {},
) {
  return useInfiniteQuery({
    queryKey: queryKeys.comics.collections(f),
    queryFn: ({ pageParam = 0 }) =>
      fetchCollections({ ...f, limit: PER_PAGE, offset: pageParam }),
...
export function useComicCollection(id: string) {
  return useQuery({
    ...
    queryFn: async (): Promise<ComicCollectionDetail> => {
      const r = await fetch(`/api/comics/collections/${id}`, {
        credentials: "include",
      });
  ...
  return useMutation({
    mutationFn: ({
      ...
      post<{ success: boolean }>(
        `/api/comics/collections/${collectionId}/series`,
        { seriesId },
      ),
    onSuccess: () => qc.invalidateQueries({ queryKey: queryKeys.comics.all }),
  });
...
export function useRemoveSeriesFromCollection() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const r = await fetch(
        `/api/comics/collections/${collectionId}/series/${seriesId}`,
